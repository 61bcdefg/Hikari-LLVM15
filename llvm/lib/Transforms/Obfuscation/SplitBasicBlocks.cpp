...
static cl::opt<size_t> s_user_split_num("split_num", cl::init(2),
...
void SplitBasicBlock::split(Function *F) {
    ...
    for (BasicBlock *currBB : origBB) {
        ...
        // Generate splits point (count number of the LLVM instructions in the current BB)
        ...
        for (size_t i = 0; i < split_ctr; ++i) {
             ...
             */
            if (F->hasFnAttribute("probe-stack") && currBB->isEntryBlock() && isa<AllocaInst>(curr_bb_it)) {
                continue;
            }

            curr_bb_offset = curr_bb_offset->splitBasicBlock(curr_bb_it, curr_bb_offset->getName() + ".split");
        }
    }
}
