      ...
      if (DecompGEP1.VarIndices.size() == 1) {
        ...
        if (Var.Val.TruncBits == 0 &&
          ...
          auto MultiplyByScaleNoWrap = [](const VariableGEPIndex &Var) {
            ...
            // If Scale is small enough so that abs(V*Scale) >= abs(Scale)
            // holds. The max value of abs(V) is 2^ValOrigBW - 1. Multiplying
            // with a constant smaller than 2^(bitwidth(Val) - ValOrigBW) won't
            // wrap.
            int MaxScaleValueBW = Var.Val.getBitWidth() - ValOrigBW;
            if (MaxScaleValueBW <= 0)
              return false;
            return Var.Scale.ule(APInt::getMaxValue(MaxScaleValueBW)
                                     .zext(Var.Scale.getBitWidth()));
          };
