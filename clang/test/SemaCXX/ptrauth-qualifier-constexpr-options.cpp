...
void test_func() {
  ...
  // expected-error@-1 2 {{empty __ptrauth authentication option}}
  ...
  // expected-error@-1 {{__ptrauth authentication option 'some'}}
  // expected-error@-2 {{missing comma between __ptrauth options}}
  // expected-error@-3 {{__ptrauth authentication option 'characters'}}
  // expected-note@-4 {{options parameter evaluated to 'some characters'}}
  ...
  // expected-error@-1 {{__ptrauth options must be a string of comma separated flags, found '2 * 3'}}
  ...
  // expected-error@-1 {{__ptrauth options must be a string of comma separated flags, found 'const_options(50)'}}
}
