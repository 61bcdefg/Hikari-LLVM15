// RUN: %clang_cc1 %s       -mllvm -ptrauth-emit-wrapper-globals=1 -fptrauth-function-pointer-type-discrimination -triple arm64e-apple-ios13 -fptrauth-calls -fptrauth-intrinsics -disable-llvm-passes -emit-llvm -o- | FileCheck %s
// RUN: %clang_cc1 -xc++ %s -mllvm -ptrauth-emit-wrapper-globals=1 -fptrauth-function-pointer-type-discrimination -triple arm64e-apple-ios13 -fptrauth-calls -fptrauth-intrinsics -disable-llvm-passes -emit-llvm -o- | FileCheck %s --check-prefixes=CHECK,CHECKCXX
...
// CHECK: @f.ptrauth = private constant { ptr, i32, i64, i64 } { ptr @f, i32 0, i64 0, i64 18983 }, section "llvm.ptrauth", align 8
...
void test_cast_to_opaque() {
  ...
  // CHECK: [[RESIGN_VAL:%.*]] = call i64 @llvm.ptrauth.resign(i64 ptrtoint (ptr @f.ptrauth to i64), i32 0, i64 18983, i32 0, i64 0)
...
void test_call_lvalue_cast() {
  ...
  // CHECK: entry:
  // CHECK-NEXT: [[RESIGN:%.*]] = call i64 @llvm.ptrauth.resign(i64 ptrtoint (ptr @f.ptrauth to i64), i32 0, i64 18983, i32 0, i64 2712)
